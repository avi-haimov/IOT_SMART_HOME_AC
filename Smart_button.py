...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self,mc):
        ...
        self.eStatus=QComboBox()
        self.eStatus.addItems(["Cool", "Heat", "Fan","Dry","Off"])

        formLayot=QFormLayout()
        formLayot.addRow("Connect",self.eConnectbtn)
        formLayot.addRow("Pub topic",self.ePublisherTopic)
        formLayot.addRow("Status",self.eStatus)
    ...
    def push_button_click(self):
        statusText = self.eStatus.currentText().upper()
        userTemp = int(self.eTemperature.text())

        # check valid status
        if(statusText=="COOL" or statusText=="HEAT" or statusText=="FAN" or
           statusText=="DRY" or statusText=="OFF"):
            self.mc.publish_to(self.ePublisherTopic.text(), statusText)
